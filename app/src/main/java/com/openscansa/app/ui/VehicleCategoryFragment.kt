...
import com.openscansa.app.databinding.FragmentVehicleCategoryBinding
...
class VehicleCategoryFragment : Fragment() {
    private var _binding: FragmentVehicleCategoryBinding? = null
    private val binding get() = _binding!!
    private val scannerViewModel: ScannerViewModel by activityViewModels()
    private val argon2 = Argon2Kt()

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        _binding = FragmentVehicleCategoryBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnStaff.setOnClickListener {
            val args = Bundle().apply {
                ...
                putString("requestKey", "plate_driver_scan")
            }
            findNavController().navigate(R.id.action_vehicleCategoryFragment_to_scannerFragment, args)
        }

        binding.btnVisitor.setOnClickListener {
            Toast.makeText(requireContext(), "Visitor flow coming soon", Toast.LENGTH_SHORT).show()
        }

        parentFragmentManager.setFragmentResultListener("plate_driver_scan", viewLifecycleOwner) { _, bundle ->
    ...
    private fun startAttendanceSession(profile: StaffRecord, warning: String?) {
        ...
        findNavController().navigate(R.id.action_vehicleCategoryFragment_to_attendanceSummaryFragment)
    }
